...
void RosJog::movel(const Eigen::Vector3d& target_position, 
  ...
  Eigen::Quaterniond current_orientation = quaternionMsgToEigen(current_pose.orientation);

  executeLinearPath(current_position, target_position,
...
void RosJog::executeLinearPath(const Eigen::Vector3d& start_position,
  ...
  auto [position_func, velocity_func] = linearTrajectory(start_position, end_position, duration);
  auto [orientation_func, _] = slerpTrajectory(start_orientation, end_orientation, duration);
...
RosJog::linearTrajectory(const Eigen::Vector3d& start, 
  ...
  auto velocity_func = [delta, duration](double t) {
